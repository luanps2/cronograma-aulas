...
export default function LabForm({ onSubmit, initialData, onCancel, isModal }) {
    ...
    const containerStyle = isModal ? {} : {
        marginBottom: '0',
        padding: '15px',
        background: initialData ? '#ECEFF1' : '#FAFAFA',
        borderRadius: '8px',
        border: initialData ? '1px solid #607D8B' : '1px solid #EEE'
    };
    ...
    return (
        <div style={containerStyle}>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
                ...
                <button className="btn-primary" style={{ flex: 1, justifyContent: 'center', backgroundColor: initialData ? '#455A64' : '' }} onClick={handleSubmit}>
                    {initialData ? <><Save size={16} /> Salvar</> : <><Plus size={16} /> Criar Lab</>}
                </button>
            </div>
        </div>
    );
